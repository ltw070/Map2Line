...
**사전 요구사항:** FastAPI 서버가 실행 중이어야 합니다.
...
브라우저에서 `http://localhost:8501` 자동 오픈 (또는 수동 접속)

### 사용 절차

1. **이미지 업로드**
   - 업로드 영역에 도면 이미지(JPG/JPEG/PNG) 드래그·드롭
   - 또는 "Browse files" 클릭하여 선택
   - 파일 크기: 최대 10MB

2. **이미지 확인**
   - 미리보기에 원본 이미지와 해상도 표시
   - 업로드된 이미지가 맞는지 확인

3. **식별 실행**
   - "API 호출 및 분석" 버튼 클릭 (주황색)
   - "분석 중..." 표시 후 결과 대기 (약 100~300ms)

4. **결과 확인**
   - **라인(Line)** — 식별된 라인명 (예: `Line_A_1`)
   - **구역(Section)** — 구역 번호 (예: `102`)
   - **신뢰도** — 0~100% 범위로 표시
   - **응답시간** — 네트워크 왕복 시간 (ms)

5. **상세 정보**
   - **기둥 좌표** — 탐지된 앵커 포인트 (픽셀 좌표)
   - **추론 통계** — API 응답 시간 / 백엔드 추론 시간
   - **원본 JSON** — 상세 응답 데이터 확인 (expandable)

### API URL 설정 (Advanced)

Sidebar의 "⚙️ 설정" 섹션에서 API 주소 변경 가능:
...
| "API 요청 시간 초과" | 서버 응답 30초 초과 | 이미지 크기 축소 또는 서버 리소스 확인 |
| "응답 파싱 오류" | API 응답 포맷 오류 | 백엔드 로그 확인 및 버전 일치 확인 |
| 파일 업로드 불가 | 형식/크기 오류 | JPG/PNG 형식, 10MB 이하로 변환
